          ...
          {slides.map((slide, index) => (
            <CarouselItem key={slide.id} className="touch-manipulation cursor-grab active:cursor-grabbing">
              ...
              >
                ...
                <div 
                  className={`absolute inset-0 flex flex-col items-end justify-end p-4 xs:p-5 sm:p-8 md:p-10 text-white text-right transition-all duration-300 ${
                    current === index ? 'opacity-100' : 'opacity-0'
                  }`}
                >
                  {/* Badge Row - Beautiful glassmorphism design with staggered animation */}
                  <div 
                    className={`flex flex-wrap items-center justify-end gap-2 xs:gap-2.5 sm:gap-3 mb-3 xs:mb-4 sm:mb-5 transition-all duration-200 ${
